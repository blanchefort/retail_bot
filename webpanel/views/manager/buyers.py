...
def index(request):
    ...
    user_profiles = Profile.objects.filter(Q(type=1) | Q(type=2))
    users = [User.objects.get(id=s.id) for s in user_profiles]
    context.update({'users': users})
    return TemplateResponse(request, 'manager/buyers.html', context=context)
...
def prolong(request, uid):
    ...
    context = {
        # Заголовок текущей страницы
        'title': 'Покупатели: платный период',
        # отображаемое на сайте имя пользователя
        'screenname': request.user.first_name or request.user.username
    }
